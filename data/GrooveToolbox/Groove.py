...
class RhythmFeatures():
    ...
    def get_density(self, part): #todo: rename to be consistent with other features? that use 1 part vs all parts functions
        ...
        non_zero_values = part[part!=0.0]
        if len(non_zero_values) == 0:
            average_velocity = 0.0
        else:
            average_velocity = non_zero_values.mean()
    ...
    def get_total_density(self):
        ...
        # Handle case where there are no non-zero values
        non_zero_values = self.groove_10_parts[self.groove_10_parts!=0.0]
        if len(non_zero_values) == 0:
            average_velocity = 0.0
        else:
            average_velocity = non_zero_values.mean()
    ...
    def get_autocorrelation_harmonicity(self):
        ...
        # Fix for Python 3.x: find_peaks returns (peaks, properties) tuple
        peaks_result = find_peaks(rectified_autocorrelation)
        peaks = peaks_result[0] + 1  # peaks = lags
