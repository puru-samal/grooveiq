...
class VQGrooveIQ_Trainer(BaseTrainer):
    """
    VQGrooveIQ (VQ-Groove Implicit Quantization Autoencoder) Trainer class that handles training, validation, and recognition loops.
    """
    def __init__(self, model : VQGrooveIQ, config : Dict, run_name : str, config_file : str, device : str = None):
        ...
        self.vq_weight     = config['loss'].get('vq_weight', 0.0)
        self.ce_weight     = config['loss'].get('ce_weight', 0.0)
        ...
        # Weight scheduler
        self.vq_weight_scheduler = None # Init it before training
        self.ce_weight_scheduler = None # Init it before training
    ...
    def set_vq_weight_scheduler(self, vq_weight_scheduler) -> None:
        self.vq_weight_scheduler = vq_weight_scheduler

    def set_ce_weight_scheduler(self, ce_weight_scheduler) -> None:
        self.ce_weight_scheduler = ce_weight_scheduler
    ...
    def _train_epoch(self, dataloader):
        ...
        # Get VQ weight
        if self.vq_weight_scheduler is not None:
            vq_weight = self.vq_weight_scheduler.get_weight()
            self.vq_weight_scheduler.step()
        else:
            vq_weight = self.vq_weight

        # Get CE weight
        if self.ce_weight_scheduler is not None:
            ce_weight = self.ce_weight_scheduler.get_weight()
            self.ce_weight_scheduler.step()
        else:
        ...
        for i, batch in enumerate(dataloader):
            ...
            running_vq_loss += vq_loss.item() * batch_size
            ...
            avg_vq_loss = running_vq_loss / running_sample_count
            avg_ce_loss = running_ce_loss / running_sample_count
        ...
        avg_metrics = {
            ...
            'vq_loss': running_vq_loss / running_sample_count,
            'ce_loss': running_ce_loss / running_sample_count,
